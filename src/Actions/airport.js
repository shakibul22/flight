...
export const getAllOneWay = async () => {
  const myHeaders = {
  ...
  try {
    const response = await axios.post("https://devapi.innotraveltech.com/flight/search", data, {
      headers: myHeaders
    });
    return response.data;
  } catch (error) {
    throw new Error(error.response?.data?.message || "Failed to perform flight search");
  }
};
